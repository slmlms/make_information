...
# 记录日志
log.to_log(config, '10kV真空断路器试验记录')
# 数据源路径
workbook_path = config.get('default', 'DataSource')
# 数据源工作表
sheet = xlrd.open_workbook(workbook_path).sheet_by_name('10kV断路器')
# 标题行
titles = data.read_titles(workbook_path, '10kV断路器')
# 保存路径
save_path_template = pathlib.Path(config.get('default', 'ten_kilovolt_circuit_breaker'))

mapping = 'ten_KV_high_voltage_vacuum_circuit_breaker'
s = set()
for i in range(1, sheet.nrows):
    # 模板路径，模板要放到循环里面，不然会生成同一个文件
    word_template = setting.get_word_template('inspection_lot\实验报告\十KV高压真空断路器.docx')
