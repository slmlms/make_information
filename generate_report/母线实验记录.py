...
# 记录日志
log.to_log(config, '母线试验记录')
# 数据源路径
workbook_path = config.get('default', 'DataSource')
# 数据源工作表
sheet = xlrd.open_workbook(workbook_path).sheet_by_name('母线实验记录')
# 标题行
titles = data.read_titles(workbook_path, '母线实验记录')
# 保存路径
save_path_template = pathlib.Path(config.get('default', 'BUSOutPutDir'))

mapping = 'BUS_test_DataMapping'
s = set()
for i in range(1, sheet.nrows):
    # 模板路径，模板要放到循环里面，不然会生成同一个文件
    word_template = setting.get_word_template('inspection_lot\实验报告\母线实验报告.docx')
