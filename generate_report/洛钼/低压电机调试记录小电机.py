    ...
    for group_name, group_data in grouped_df:
        ...
        if rows_count <= 29:
            ...
            for index, row in group_data.iterrows():
                ...
                ws["O" + str(current_row)].value = row['功率因数']
...
def convert_to_pdf(input_file, output_file):
    cmd = ["libreoffice", "--headless", "--convert-to", "pdf", input_file, "--outdir", os.path.dirname(output_file)]
    subprocess.run(cmd)

# 合并PDF文件
def merge_pdfs(input_files, output_file):
    cmd = ["libreoffice", "--headless", "--convert-to", "pdf", "--outdir", os.path.dirname(output_file)] + input_files
    subprocess.run(cmd)
...
def main():
    ...
    output_path = fill_template(grouped_df, template_path,folder_name)
    # 创建文件夹并保存文件
