...
# 记录日志
log.to_log(config, '软启实验记录')
# 数据源路径
workbook_path = config.get('default', 'DataSource')
# 数据源工作表
sheet = xlrd.open_workbook(workbook_path).sheet_by_name('软启实验记录')
# 标题行
titles = data.read_titles(workbook_path, '软启实验记录')
# 保存路径
save_path_template = pathlib.Path(config.get('default', 'softStarterOutPutDir'))

mapping = 'soft_starter'

s = set()
for i in range(1, sheet.nrows):
    # 模板路径，模板要放到循环里面，不然会生成同一个文件
    word_template = setting.get_word_template('inspection_lot\实验报告\低压软启柜实验报告模板.docx')
